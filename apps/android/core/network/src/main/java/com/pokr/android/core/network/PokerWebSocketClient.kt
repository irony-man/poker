...
class PokerWebSocketClient(
    ...
    private val _opened = MutableSharedFlow<Unit>(extraBufferCapacity = 8)
    ...
    /** Keep an existing session socket; only open one if needed. */
    fun ensureConnected(wsUrl: String = defaultWsUrl) {
        if (isOpen && shouldReconnect && (currentWsUrl == null || currentWsUrl == wsUrl)) return
        connect(wsUrl)
    }
