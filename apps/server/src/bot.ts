...
export function chooseBotAction(
  ...
  const tryRaiseOrBet = (prefer: 'bet' | 'raise'): ActionIntent | null => {
    ...
    // Usually open ~½–¾ pot-ish; sometimes jam short stacks
  ...
  // Free option: check more often than bet
  if (types.has('check')) {
    if (r < 0.28) {
  ...
  // Fallback: first legal non-fold if possible
