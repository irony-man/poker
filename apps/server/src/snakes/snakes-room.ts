...
import {
  ...
  startMatch,
  type SnakesState,
} from '@poker/snakes-engine';
import type { SnakesPublicView } from '@poker/protocol';
...
export const SNAKES_INACTIVITY_MS = 15 * 60 * 1000;
export const SNAKES_IDLE_SWEEP_MS = 30_000;
const DISCONNECT_GRACE_MS = 120_000;
const CHAT_BUFFER = 80;
const BOT_TURN_MIN_MS = 450;
const BOT_TURN_MAX_MS = 1100;

export interface SnakesMeta {
  id: string;
  inviteCode: string;
  name: string;
  hostUserId: string;
...
export interface SnakesConnection {
  userId: string;
  name: string;
  avatarId: number;
...
export interface SnakesChatRow {
  userId: string;
  name: string;
  text: string;
  at: number;
...
function matchStatus(phase: SnakesState['phase']): SnakesPublicView['status'] {
...
export class SnakesRoom {
  meta: SnakesMeta;
  state: SnakesState;
  private connections = new Map<string, SnakesConnection>();
  ...
  private chatLog: SnakesChatRow[] = [];
  private botNamePool: string[] | null = null;
  private botStyles: BotStyleOptions | null = null;
  private idleSince: number | null;
  private closed = false;

  constructor(meta: SnakesMeta, turnTimeMs?: number) {
    this.meta = meta;
    this.state = createMatch({ matchId: meta.id, maxSeats: meta.maxSeats, turnTimeMs });
  ...
  attach(conn: SnakesConnection): void {
    if (this.closed) {
  ...
  isActiveConnection(userId: string, send: SnakesConnection['send']): boolean {
    return this.connections.get(userId)?.send === send;
  }

  detachIfActive(userId: string, send: SnakesConnection['send']): boolean {
  ...
  roll(userId: string, seq?: number): { ok: boolean; error?: string } {
    ...
    if (!result.ok) return { ok: false, error: result.error };
    this.state = result.state;
    if (result.state.winnerSeat != null) {
      const winner = this.state.seats[result.state.winnerSeat]?.name ?? 'Player';
  ...
  isIdle(ms: number = SNAKES_INACTIVITY_MS, now: number = Date.now()): boolean {
  ...
  private armTurnTimer(): void {
    this.clearTurnTimer();
    if (this.state.toAct === null || this.state.phase !== 'rolling') return;
  ...
  private runBotTurn(seat: number): void {
    ...
    if (intent.type === 'roll') {
      ...
      if (result.ok) {
        this.state = result.state;
        if (result.state.winnerSeat != null) {
          this.systemChat(`${this.state.seats[result.state.winnerSeat]?.name ?? 'Bot'} wins`);
        }
        this.afterStateChange();
      }
    }
  ...
  private onHumanTimeout(seat: number): void {
    if (this.state.toAct !== seat || this.state.phase !== 'rolling') return;
    const rolled = roll(this.state, seat);
    if (!rolled.ok) return;
    this.state = rolled.state;
    if (rolled.state.winnerSeat != null) {
      this.systemChat(`${this.state.seats[rolled.state.winnerSeat]?.name ?? 'Player'} wins`);
    }
    this.afterStateChange();
  }
  ...
  private pushChat(userId: string, name: string, text: string): void {
    ...
    const msg = {
      type: 'snakes_chat' as const,
      snakesId: this.meta.id,
      userId,
      name,
      text,
      at,
  ...
  private pushTo(userId: string): void {
    ...
    if (!conn) return;
    conn.send({
      type: 'snakes_state_sync',
      snakes: this.toPublicView(),
...
export class SnakesRoomManager {
  private rooms = new Map<string, SnakesRoom>();
  ...
  create(opts: {
    ...
    this.rooms.set(id, new SnakesRoom(meta, opts.turnTimeMs));
    this.byInvite.set(inviteCode, id);
    return meta;
  }

  get(id: string): SnakesRoom | undefined {
    return this.rooms.get(id);
  }

  getByInvite(code: string): SnakesRoom | undefined {
  ...
  terminateIdleRooms(now: number = Date.now(), inactivityMs: number = SNAKES_INACTIVITY_MS): string[] {
    const closed: string[] = [];
