...
import { CourtpieceView } from '@/components/courtpiece/CourtpieceView';
...
function CourtpiecePlayInner() {
  ...
  useEffect(() => {
    ...
    const key = `${courtpieceId}:${stored.userId}`;
  ...
  if (needsAuth) {
    const returnTo = `/courtpiece/${courtpieceId}${invite ? `?invite=${invite}` : ''}${
      spectate ? `${invite ? '&' : '?'}mode=spectate` : ''
    }`;
    return (
      <div className="hud-panel mx-auto max-w-md space-y-4 p-6">
        <h2 className="font-display text-xl uppercase tracking-wider text-on-chrome">
          Sign in to join
        </h2>
        <p className="text-sm text-on-chrome/70">You need an account to enter this Court Piece board.</p>
        {error ? (
  ...
  return (
    <CourtpieceView courtpieceId={courtpieceId} inviteCode={invite} initialSpectate={spectate} />
  );
}

export default function CourtpiecePlayPage() {
  return (
