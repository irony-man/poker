...
function MemoryPlayInner() {
  ...
  useEffect(() => {
    ...
    const key = `${memoryId}:${stored.userId}`;
  ...
  if (needsAuth) {
    const returnTo = `/memory/${memoryId}${invite ? `?invite=${invite}` : ''}${
    ...
    return (
      <div className="hud-panel mx-auto max-w-md space-y-4 p-6">
        ...
        <p className="text-sm text-on-chrome/70">You need an account to enter this Memory board.</p>
        {error ? (
  ...
  return <MemoryView memoryId={memoryId} inviteCode={invite} initialSpectate={spectate} />;
}
