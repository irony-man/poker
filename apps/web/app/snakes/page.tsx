...
import { createSnakes } from '@/lib/api';
...
export default function SnakesHostPage() {
  const router = useRouter();
  const { authReady, signedIn, sessionToken, ensureSession } = useLobbySession();
  const pageCopy = usePageCopy('snakes');
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const board = await createSnakes(
        {
      ...
      router.push(`/snakes/${board.snakesId}?invite=${board.inviteCode}`);
    } catch (err) {
          ...
          <FriendInvitePicker
            ...
            help={
              maxFriendInvites === 0
                ...
                : 'They get a Snakes invite in Friends. Optional — share the room code too.'
            }
          />
