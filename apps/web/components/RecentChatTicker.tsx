...
export function RecentChatTicker({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <button
      ...
      className="recent-chat-ticker glass-sheet flex max-h-[4.5rem] w-full min-w-0 flex-col justify-end gap-0.5 overflow-hidden rounded-xl border border-sidebar/10 px-2.5 py-1.5 text-left shadow-[0_4px_14px_rgb(29_4_50/0.08)] backdrop-blur-sm"
    >
      {lines.map((m, i) => (
        <p
          key={`${m.at}-${m.userId}-${i}`}
